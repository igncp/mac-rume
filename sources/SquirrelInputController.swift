...
final class SquirrelInputController: IMKInputController {
  ...
  private var chordKeyCodes: [UInt32] = .init(
    repeating: 0,
    count: SquirrelInputController.keyRollOver
  )
  ...
  override func handle(_ event: NSEvent!, client sender: Any!) -> Bool {
    ...
    case .flagsChanged:
      ...
      var rimeModifiers: UInt32 = SquirrelKeycode.osxModifiersToRime(
        modifiers: modifiers
      )
      ...
      for flag in [
        ...
        if modifiers.contains(flag) {  // New modifier
          buffer.append(
            (keycode: rimeKeycode, modifier: rimeModifiers)
          )
        } else {  // Release
          buffer.insert(
            (
              keycode: rimeKeycode,
              modifier: rimeModifiers | kReleaseMask.rawValue
            ),
            at: 0
          )
        }
    ...
    case .keyDown:
      ...
      if let code = keyChars?.first,
        (capitalModifiers && !code.isLetter)
          || (!capitalModifiers && !code.isASCII)
      {
  ...
  override func recognizedEvents(_ sender: Any!) -> Int {
    return Int(
      NSEvent.EventTypeMask.Element(arrayLiteral: .keyDown, .flagsChanged)
        .rawValue
    )
  }
  ...
  // Called when a user action was taken that ends an input session.
  ...
  override func menu() -> NSMenu! {
    let deploy = NSMenuItem(
      title: NSLocalizedString("Deploy", comment: "Menu item"),
    ...
    let setting = NSMenuItem(
      ...
      action: #selector(openRimeFolder),
      keyEquivalent: ""
    )
    setting.target = self
    let wiki = NSMenuItem(
...
extension SquirrelInputController {
  ...
  fileprivate func updateChord(keycode: UInt32, modifiers: UInt32) {
    ...
    chordTimer = Timer.scheduledTimer(
      withTimeInterval: chordDuration,
      repeats: false,
      block: onChordTimer
    )
  }
  ...
  fileprivate func updateAppOptions() {
    ...
    if let appOptions = NSApp.squirrelAppDelegate.config?.getAppOptions(
      currentApp
    ) {
  ...
  fileprivate func processKey(
    ...
    if !handled {
      ...
      let isChordingKey =
        ...
        case XK_space...XK_asciitilde, XK_Control_L, XK_Control_R,
  ...
  fileprivate func rimeUpdate() {
    ...
    if rimeAPI.get_context(session, &ctx) {
      ...
      let start =
        String.Index(
          preedit.utf8.index(
            preedit.utf8.startIndex,
            offsetBy: Int(ctx.composition.sel_start)
          ),
          within: preedit
        ) ?? preedit.startIndex
      let end =
        String.Index(
          preedit.utf8.index(
            preedit.utf8.startIndex,
            offsetBy: Int(ctx.composition.sel_end)
          ),
          within: preedit
        ) ?? preedit.startIndex
      let caretPos =
        String.Index(
          preedit.utf8.index(
            preedit.utf8.startIndex,
            offsetBy: Int(ctx.composition.cursor_pos)
          ),
          within: preedit
        ) ?? preedit.startIndex

      if inlineCandidate {
        var candidatePreview =
          ctx.commit_text_preview.map { String(cString: $0) } ?? ""
        ...
        let caretPos =
          caretPos <= start ? caretPos : endOfCandidatePreview
        show(
      ...
      if !rimeAPI.get_option(session, "_hide_candidate") {
        for i in 0..<numCandidates {
          ...
          candidates.append(
            candidate.text.map { String(cString: $0) } ?? ""
          )
          comments.append(
            candidate.comment.map { String(cString: $0) } ?? ""
          )
        }
        ...
        if let select_keys = ctx.menu.select_keys {
          ...
          for i in 0..<pageSize {
            labels.append(
              select_labels[i].map { String(cString: $0) } ?? ""
            )
          }
        }
  ...
  fileprivate func show(preedit: String, selRange: NSRange, caretPos: Int) {
    ...
    let remainingRange = NSRange(
      location: start,
      length: preedit.utf16.count - start
    )
    let attrs =
